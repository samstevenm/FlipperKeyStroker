...
const UsbHidstrokerKey usb_hid_stroker_keyset[ROW_COUNT][COLUMN_COUNT] = {
    {
...
static void usb_hid_stroker_to_upper(char* str) {
    while(*str) {
...
static void usb_hid_stroker_draw_key(
    Canvas* canvas,
    UsbHidstrokerModel* model,
    uint8_t x,
    uint8_t y,
    UsbHidstrokerKey key,
    bool selected) {
    if(!key.width) return;
    ...
    if(key.icon != NULL) {
        ...
        if((model->ctrl && key.value == HID_KEYBOARD_L_CTRL) ||
           ...
           (model->gui && key.value == HID_KEYBOARD_L_GUI)) {
            usb_hid_stroker_to_upper(model->key_string);
        }
...
static void usb_hid_stroker_draw_callback(Canvas* canvas, void* context) {
    furi_assert(context);
    UsbHidstrokerModel* model = context;
    ...
    for(uint8_t y = initY; y < ROW_COUNT; y++) {
        const UsbHidstrokerKey* keyboardKeyRow = usb_hid_stroker_keyset[y];
        uint8_t x = 0;
        for(uint8_t i = 0; i < COLUMN_COUNT; i++) {
            UsbHidstrokerKey key = keyboardKeyRow[i];
...
static uint8_t usb_hid_stroker_get_selected_key(UsbHidstrokerModel* model) {
    UsbHidstrokerKey key = usb_hid_stroker_keyset[model->y][model->x];
...
static void
    usb_hid_stroker_get_select_key(UsbHidstrokerModel* model, UsbHidstrokerPoint delta) {
    ...
    } while(delta.y != 0 && usb_hid_stroker_keyset[model->y][model->x].value == 0);
    ...
    } while(delta.x != 0 && usb_hid_stroker_keyset[model->y][model->x].width ==
                                0); // Skip zero width keys, pretend they are one key
}

static void usb_hid_stroker_process(UsbHidstroker* usb_hid_stroker, InputEvent* event) {
    with_view_model(
        usb_hid_stroker->view,
        UsbHidstrokerModel * model,
        {
            if(event->key == InputKeyOk) {
                if(event->type == InputTypePress) {
                    ...
                    model->last_key_code = usb_hid_stroker_get_selected_key(model);
                ...
                if(event->key == InputKeyUp) {
                    usb_hid_stroker_get_select_key(model, (UsbHidstrokerPoint){.x = 0, .y = -1});
                } else if(event->key == InputKeyDown) {
                    usb_hid_stroker_get_select_key(model, (UsbHidstrokerPoint){.x = 0, .y = 1});
                } else if(event->key == InputKeyLeft) {
                    usb_hid_stroker_get_select_key(model, (UsbHidstrokerPoint){.x = -1, .y = 0});
...
static bool usb_hid_stroker_input_callback(InputEvent* event, void* context) {
    furi_assert(context);
    UsbHidstroker* usb_hid_stroker = context;
    bool consumed = false;

    if(event->type == InputTypeLong && event->key == InputKeyBack) {
        furi_hal_hid_kb_release_all();
    } else {
        usb_hid_stroker_process(usb_hid_stroker, event);
        consumed = true;
    }
...
UsbHidstroker* usb_hid_stroker_alloc() {
    UsbHidstroker* usb_hid_stroker = malloc(sizeof(UsbHidstroker));

    usb_hid_stroker->view = view_alloc();
    view_set_context(usb_hid_stroker->view, usb_hid_stroker);
    view_allocate_model(usb_hid_stroker->view, ViewModelTypeLocking, sizeof(UsbHidstrokerModel));
    view_set_draw_callback(usb_hid_stroker->view, usb_hid_stroker_draw_callback);
    view_set_input_callback(usb_hid_stroker->view, usb_hid_stroker_input_callback);

    with_view_model(
        usb_hid_stroker->view, UsbHidstrokerModel * model, { model->connected = true; }, true);

    return usb_hid_stroker;
}

void usb_hid_stroker_free(UsbHidstroker* usb_hid_stroker) {
    furi_assert(usb_hid_stroker);
    view_free(usb_hid_stroker->view);
    free(usb_hid_stroker);
}

View* usb_hid_stroker_get_view(UsbHidstroker* usb_hid_stroker) {
    furi_assert(usb_hid_stroker);
    return usb_hid_stroker->view;
}
